...
describe Board do
  ...
  describe "#make_move" do
    ...
    # DUE TO A LACK OF SLEEP, I MUST STOP
    ...
    context "when the turn ended on a cup with stones already in it" do
      xit "returns the cup's array index" do
  ...
  describe "#cups_empty?" do
    xit "returns true if all of the non-point cups are empty" do
      [0..5].each { |idx| board.cups[idx] = [] }
      expect(board.cups_empty?).to be(true)
    end

    xit "returns false if all of the non-point cups are not empty" do
  ...
  describe "#winner" do
    xit "returns :draw if the game was tied" do
      board.cups[6] = six_stone_cup
      board.cups[13] = six_stone_cup
      expect(board.winner).to eq(:draw)
    end

    xit "returns the winner's name if the game was won" do
      board.cups[6] = six_stone_cup
