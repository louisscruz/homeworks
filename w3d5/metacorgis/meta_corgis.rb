...
class MetaCorgiPerkPackage
  ...
  #   resource = name.to_s
  #   info = @shopping_list.send("get_#{resource}_info", @package_id)
  #   happiness = @shopping_list.send("get_#{resource}_happiness", @package_id)
  #   resource = resource.split('_').map(&:capitalize).join(' ')
  #   result = "#{resource}: #{info}: #{happiness} licks"
  #   happiness > 30 ? (puts "* #{result}") : (puts result)
  # end

  def self.define_perk(name)
    define_method(name) do
      resource = name.to_s
      info = @shopping_list.send("get_#{resource}_info", @id)
      happiness = @shopping_list.send("get_#{resource}_happiness", @id)
      resource = resource.split('_').map(&:capitalize).join(' ')
      result = "#{resource}: #{info}: #{happiness} licks"
      happiness > 30 ? (puts "* #{result}") : (puts result)
    end
